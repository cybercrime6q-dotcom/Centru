...
.stab-icon{margin-right:4px;font-size:14px;}
...
.status-ring.unseen{border-color:var(--st);}
...
/* ===== CHAT HEADER ===== */
.chat-header{height:60px;background:var(--pn);display:flex;align-items:center;gap:8px;
  padding:0 10px 0 12px;border-bottom:1px solid var(--bd);flex-shrink:0;}
...
.header-info{flex:1;cursor:pointer;min-width:0;}
.header-info h3{font-weight:900;font-size:15px;color:var(--tx);}
.header-info p{font-size:11px;color:var(--g);}
.header-btn{width:40px;height:40px;border-radius:50%;background:transparent;color:var(--st);
...
.bubble-time{font-size:10.5px;color:rgba(255,255,255,.45);text-align:right;margin-top:2px;
  display:flex;align-items:center;justify-content:flex-end;gap:3px;white-space:nowrap;}
.tick{font-size:12px;}.tick.read{color:var(--bl);}
...
.call-cbtn{width:64px;height:64px;border-radius:50%;border:none;display:flex;align-items:center;justify-content:center;font-size:26px;cursor:pointer;transition:.2s;flex-direction:column;gap:4px;}
.call-cbtn:hover{transform:scale(1.08);}
.call-cbtn span.lbl{font-size:10px;font-weight:700;color:rgba(255,255,255,.55);margin-top:4px;}
...
<div class="sb" id="sidebar">
  ...
  <!-- 3 Sidebar Tabs: Chat | Status | Kontak -->
  <div class="sbtabs">
    <div class="stab active" onclick="switchTab('chats')" id="tab-chats"><span class="stab-icon">&#128172;</span>Chat</div>
    <div class="stab" onclick="switchTab('status')" id="tab-status"><span class="stab-icon">&#128994;</span>Status</div>
    <div class="stab" onclick="switchTab('contacts')" id="tab-contacts"><span class="stab-icon">&#128101;</span>Kontak</div>
  </div>

  <!-- Search (hidden on status tab) -->
  ...
  <div class="sb-panel active" id="panel-chats">
    <div id="chat-list">
      <div style="padding:28px;text-align:center;color:var(--st);font-size:14px;">Memuat...</div>
    </div>
  </div>
  ...
  <div class="sb-panel" id="panel-status">
    ...
    <div id="status-empty" style="display:none;padding:30px;text-align:center;color:var(--st);font-size:13px;">
      Belum ada status teman ðŸ‘€
    </div>
  </div>
...
<div class="main" id="main-panel">
  ...
  <div class="no-chat" id="no-chat" style="display:flex;">
    ...
    <div style="margin-top:16px;display:flex;gap:10px;">
      <button onclick="openAI()" style="padding:10px 20px;background:linear-gradient(135deg,#7c3aed,#9b5de5);color:#fff;border:none;border-radius:20px;font-size:14px;font-weight:800;cursor:pointer;font-family:'Nunito',sans-serif;">
        Buka AI
      </button>
      <button onclick="switchTab('status')" style="padding:10px 20px;background:var(--pn);color:var(--tx);border:1.5px solid var(--bd);border-radius:20px;font-size:14px;font-weight:800;cursor:pointer;font-family:'Nunito',sans-serif;">
        Lihat Status
      </button>
    </div>
  </div>
  ...
  <div id="chat-wrap" style="display:none;flex-direction:column;height:100%;overflow:hidden;">
    ...
    <div class="chat-header">
      ...
      <!-- AI in chat header -->
      <button class="header-btn" onclick="openAI()" title="Tanya AI" style="color:#9b5de5;">
        <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M12 2a2 2 0 012 2c0 .74-.4 1.39-1 1.73V7h1a7 7 0 017 7h1a1 1 0 010 2h-1v1a2 2 0 01-2 2H5a2 2 0 01-2-2v-1H2a1 1 0 010-2h1a7 7 0 017-7h1V5.73A2 2 0 0110 4a2 2 0 012-2zm-5 9a5 5 0 000 10h10a5 5 0 000-10H7zm2 3a1 1 0 110 2 1 1 0 010-2zm6 0a1 1 0 110 2 1 1 0 010-2z"/></svg>
      </button>
      <!-- Audio call -->
      <button class="header-btn" onclick="startCall('audio')" title="Telepon">
      ...
      <!-- Video call -->
      <button class="header-btn" onclick="startCall('video')" title="Video Call">
...
<div class="ai-drawer" id="ai-drawer">
  <div class="ai-drawer-header">
    ...
    <div style="flex:1;">
      ...
      <div style="font-size:11px;color:rgba(224,212,255,.5);" id="ai-sub">Asisten pintarmu</div>
    </div>
  ...
  <div class="ai-messages" id="ai-messages">
    <div class="ai-msg ai">
      <div class="ai-msg-av">ðŸ¤–</div>
      <div class="ai-bubble">
        Halo! Saya <b>WaClone AI</b> ðŸ‘‹<br><br>
        Saya bisa membantu kamu:<br>
        â€¢ Menulis atau menyempurnakan pesan<br>
        â€¢ Menjawab pertanyaan apapun<br>
        â€¢ Menerjemahkan teks<br>
        â€¢ Memberi ide & saran kreatif<br><br>
  ...
  <div class="ai-chips" id="ai-chips">
    <div class="ai-chip" onclick="sendAIMsg('Bantu saya menulis pesan yang baik untuk teman')">âœï¸ Tulis pesan</div>
    <div class="ai-chip" onclick="sendAIMsg('Buat lelucon lucu yang bisa dikirim ke teman')">ðŸ˜‚ Lelucon</div>
    <div class="ai-chip" onclick="sendAIMsg('Apa tips berkomunikasi yang baik?')">ðŸ’¡ Tips chat</div>
    <div class="ai-chip" onclick="sendAIMsg('Terjemahkan ke bahasa Inggris: Halo, apa kabar?')">ðŸŒ Terjemah</div>
  </div>
  <div class="ai-input-row">
    <textarea class="ai-textarea" id="ai-input" rows="1"
      placeholder="Tanya AI apapun..."
...
<div class="overlay" id="notif-ov">
  <div class="panel">
    ...
    <div class="panel-body" id="notif-list">
      <div style="text-align:center;color:var(--st);padding:22px;">Tidak ada notifikasi ðŸŽ‰</div>
    </div>
  </div>
</div>
...
<div class="overlay" id="stview-ov">
  <div class="stv" id="stv-wrap">
    ...
    <div class="stv-head">
      ...
      <button class="close-btn" onclick="closeStatusViewer()" style="background:rgba(255,255,255,.1);color:rgba(255,255,255,.7);">âœ•</button>
    </div>
...
<div class="call-ui" id="call-ui">
  ...
  <div class="call-video-grid" id="call-video-grid" style="display:none;">
    <div class="remote-vid-wrap">
      ...
      <div class="cam-off-label" id="remote-cam-off">
        <div style="font-size:50px;" id="remote-av-placeholder">ðŸ‘¤</div>
  ...
  <div class="call-controls">
    <div style="display:flex;flex-direction:column;align-items:center;gap:2px;">
      <button class="call-cbtn cbtn-toggle cbtn-on" id="cbtn-mic" onclick="toggleMic()">ðŸŽ¤</button>
      <span style="font-size:10px;color:rgba(255,255,255,.5);">Mic</span>
    </div>
    <div style="display:flex;flex-direction:column;align-items:center;gap:2px;" id="cam-ctrl-wrap" style="display:none;">
      <button class="call-cbtn cbtn-toggle cbtn-on" id="cbtn-cam" onclick="toggleCam()">ðŸ“·</button>
      <span style="font-size:10px;color:rgba(255,255,255,.5);">Kamera</span>
    </div>
    <div style="display:flex;flex-direction:column;align-items:center;gap:2px;">
      <button class="call-cbtn cbtn-end" onclick="endCall()">ðŸ“µ</button>
      <span style="font-size:10px;color:rgba(255,255,255,.5);">Tutup</span>
    </div>
    <div style="display:flex;flex-direction:column;align-items:center;gap:2px;">
      <button class="call-cbtn cbtn-toggle cbtn-on" id="cbtn-spk" onclick="toggleSpeaker()">ðŸ”Š</button>
...
<div class="incoming-call" id="incoming-call">
  ...
  <div class="inc-type-badge" id="inc-type">ðŸ“ž Panggilan Masuk</div>
  <div class="inc-actions">
    <button class="inc-btn inc-audio" onclick="answerCall('audio')">ðŸ“ž Audio</button>
...
function openPanel(id){
  ...
  // Track overlay in nav stack (only .overlay elements)
  const el=document.getElementById(id);
  if(el&&el.classList.contains('overlay')) navPush({type:'overlay',id});
}
...
function renderChatList(){
  ...
  if(!others.length){el.innerHTML='<div style="padding:28px;text-align:center;color:var(--st);font-size:14px;">Belum ada pengguna ðŸ‘¥</div>';return;}
...
function openChat(uid,name,avatar,bio){
  ...
  navPush({type:'chat',uid,name,avatar,bio});
...
// goBack is defined below in navigation stack section
...
async function loadMessages(){
  ...
  try{
    ...
    if(f){
      ...
      st.textContent=f.online?'ðŸŸ¢ Online':(f.last_seen?`Terakhir ${fmtTime(f.last_seen)}`:'âš« Offline');
    }
  }catch(e){}
}
...
function renderMessages(msgs){
  ...
  if(!msgs.length){area.innerHTML='<div style="text-align:center;color:var(--st);padding:36px;font-size:13px;">Belum ada pesan. Mulai percakapan! ðŸ‘‹</div>';return;}
  let html='',lastDate='';
  msgs.forEach(m=>{
    ...
    if(m.file){
      ...
      else{const fn=m.file.split('/').pop().split('?')[0].substring(0,30);content+=`<a class="file-link" href="${m.file}" target="_blank">ðŸ“„ ${escHtml(fn)}</a>`;}
    }
    ...
    const acts=`<div class="msg-actions">
      <button class="act-btn" onclick='setReply("${m.id}","${safeTxt}","${m.from}")' title="Balas">â†©</button>
      <button class="act-btn" onclick='showCtxMenu(event,"${m.id}","${safeTxt}","${m.from}")' title="Lebih">â‹¯</button>
    </div>`;
...
async function sendMessage(){
  ...
  try{
    ...
    const d=await r.json();
    if(d.ok) loadMessages(); else toast('Gagal kirim ðŸ˜•',2500,true);
  }catch(e){toast('Gagal kirim ðŸ˜•',2500,true);}
}
...
async function execForward(){
  ...
  const d=await r.json();
  if(d.ok){toast('Diteruskan â†ªï¸');closePanel('fw-ov');}else toast('Gagal',2500,true);
}
async function doDelete(){
  ...
  const d=await r.json();
  if(d.ok){loadMessages();toast('Dihapus ðŸ—‘ï¸');}else toast('Gagal hapus',2500,true);
}
...
async function handleUpload(input){
  ...
  try{
    ...
    if(d.ok){toast('Terkirim ðŸ“Ž');cancelReply();loadMessages();}else toast('Gagal: '+(d.msg||''),3000,true);
...
async function sendCamPhoto(){
  ...
  if(camMode==='status'){
    ...
    if(d.ok){toast('Status diposting âœ…');loadSidebarStatuses();}else toast('Gagal',2500,true);
  }else{
    if(!currentFriend){done();return;}
    fd.append('to_uid',currentFriend.uid);
    const r=await fetch('/api/send_file',{method:'POST',body:fd}); const d=await r.json(); done();
    if(d.ok){toast('Foto terkirim ðŸ“¸');loadMessages();}else toast('Gagal',2500,true);
  }
}
  ...
  try{
    ...
    mediaRecorder.onstop=async()=>{
      ...
      if(blob.size>500&&currentFriend){
        ...
        const r=await fetch('/api/send_file',{method:'POST',body:fd}); const d=await r.json(); done();
        if(d.ok){toast('Suara terkirim ðŸŽ™ï¸');loadMessages();}else toast('Gagal kirim suara',2500,true);
      }
    };
    mediaRecorder.start(); document.getElementById('rec-btn').classList.add('recording'); toast('ðŸŽ™ï¸ Merekam... Lepaskan untuk kirim',15000);
...
async function loadSidebarStatuses(){
  try{
    ...
    document.getElementById('my-status-hint').textContent=
      myStats.length?`${myStats.length} status Â· ${fmtTime(myStats[myStats.length-1].time)}`:'Ketuk untuk lihat atau buat status';
    // Friends
    ...
    fl.innerHTML=entries.map(([uid,sts])=>{
      ...
      return `<div class="status-friend-item" onclick="viewFriendStatus('${uid}')">
        <div class="status-ring-wrap">
          <div class="status-ring"><div class="status-ring-inner">${makeAv(u,44)}</div></div>
        </div>
        <div style="flex:1;min-width:0;margin-left:12px;">
          <div style="font-weight:800;font-size:14px;">${escHtml(u.username)}</div>
          <div style="font-size:12px;color:var(--st);margin-top:1px;">${fmtTime(latest.time)} Â· ${sts.length} status</div>
        </div>
...
async function openMyStatus(){
  ...
  if(!myStats.length){openPanel('cstat-ov');navPush({type:'overlay',id:'cstat-ov'});return;}
  const u={uid:ME.uid,username:ME.username,avatar:''};
  stvData={user:u,statuses:myStats}; stvIdx=0; renderSTV(); openPanel('stview-ov'); navPush({type:'overlay',id:'stview-ov'});
}
...
async function viewFriendStatus(uid){
  ...
  const u=allUsers.find(x=>x.uid===uid)||{uid,username:'?',avatar:''};
  stvData={user:u,statuses:d.statuses}; stvIdx=0; renderSTV(); openPanel('stview-ov'); navPush({type:'overlay',id:'stview-ov'});
}
...
function renderSTV(){
  ...
  if(s.type==='text'){
    ...
    body=`<img class="stv-img" src="${s.media_url}" alt="status" onerror="this.style.display='none'">`;
  ...
  document.getElementById('stv-next-btn').textContent=stvIdx>=statuses.length-1?'Tutup âœ•':'Next â–¶';
  ...
  stvTimerInt=setInterval(()=>{
    if(stvIdx<statuses.length-1){stvIdx++;renderSTV();}
    else{closeStatusViewer();}
  },5000);
}
...
async function postTextStatus(){
  ...
  const d=await r.json();
  if(d.ok){toast('Status diposting âœ…');closePanel('cstat-ov');document.getElementById('stat-text-inp').value='';loadSidebarStatuses();}
  else toast('Gagal: '+(d.msg||''),3000,true);
}
async function uploadStatus(input,type){
  if(!input.files[0])return;
  const done=showUploadBar(),fd=new FormData(); fd.append('file',input.files[0]); fd.append('type',type);
  const r=await fetch('/api/status/upload',{method:'POST',body:fd}); const d=await r.json(); done();
  if(d.ok){toast('Status diposting âœ…');closePanel('cstat-ov');loadSidebarStatuses();}else toast('Gagal',3000,true);
  input.value='';
}
...
async function saveProfile(){
  ...
  if(d.ok){document.getElementById('pname').textContent=u;toast('Profil disimpan âœ…');closePanel('prof-ov');setTimeout(()=>location.reload(),800);}
  else toast('Gagal: '+(d.msg||''),3000,true);
}
...
async function loadNotifications(){
  ...
  if(!notifs.length){el.innerHTML='<div style="text-align:center;color:var(--st);padding:22px;">Tidak ada notifikasi ðŸŽ‰</div>';return;}
...
async function startCall(type){
  ...
  if(type==='video'){document.getElementById('localVid').srcObject=localStream;toggleVideoLayout(true);}
  pc.ontrack=e=>{document.getElementById('remoteVid').srcObject=e.streams[0];document.getElementById('remote-cam-off').classList.remove('show');};
...
function showCallUI(friend,type,dir){
  ...
  const camWrap=document.getElementById('cam-ctrl-wrap');
  camWrap.style.display=type==='video'?'flex':'none';
  micMuted=false; camOff=false;
  document.getElementById('cbtn-mic').textContent='ðŸŽ¤'; document.getElementById('cbtn-mic').className='call-cbtn cbtn-toggle cbtn-on';
  document.getElementById('cbtn-cam').textContent='ðŸ“·'; document.getElementById('cbtn-cam').className='call-cbtn cbtn-toggle cbtn-on';
}

function toggleVideoLayout(show){
  document.getElementById('call-video-grid').style.display=show?'flex':'none';
  document.getElementById('audio-call-info').style.display=show?'none':'flex';
}
...
function endCall(){
  ...
  toggleVideoLayout(false); callSecs=0;
}
...
function toggleSpeaker(){
  ...
  btn.textContent=speakerOff?'ðŸ”‡':'ðŸ”Š';
  btn.className='call-cbtn cbtn-toggle '+(speakerOff?'cbtn-off-state':'cbtn-on');
  toast(speakerOff?'Speaker dimatikan ðŸ”‡':'Speaker aktif ðŸ”Š',1500);
}
...
async function sendAIMsg(text){
  ...
  uDiv.innerHTML=`<div class="ai-msg-av">ðŸ‘¤</div><div class="ai-bubble">${escHtml(text)}</div>`;
  container.appendChild(uDiv);
  aiHistory.push({role:'user',content:text});
  // Thinking bubble
  const thinkDiv=document.createElement('div'); thinkDiv.className='ai-msg ai'; thinkDiv.id='ai-thinking';
  thinkDiv.innerHTML=`<div class="ai-msg-av">ðŸ¤–</div><div class="ai-thinking-bubble"><span></span><span></span><span></span></div>`;
  container.appendChild(thinkDiv);
  container.scrollTop=container.scrollHeight;

  try{
    // System prompt as separate parameter (correct Anthropic API format)
    ...
    sys+='Jika diminta menulis pesan untuk dikirim ke teman, buat pesan yang sesuai dan alami. Jawab semua pertanyaan pengguna dengan baik dan lengkap.';

    // Build messages - must alternate user/assistant
    const msgs=[];
    for(const m of aiHistory.slice(-14)){
      if(!msgs.length||msgs[msgs.length-1].role!==m.role) msgs.push(m);
      else msgs[msgs.length-1]={role:m.role,content:msgs[msgs.length-1].content+'\n'+m.content};
    }
    // Must start with user
    const startIdx=msgs.findIndex(m=>m.role==='user');
    const finalMsgs=startIdx>=0?msgs.slice(startIdx):msgs;
    ...
    const res=await fetch('/api/ai/chat',{
      ...
      body:JSON.stringify({
        system:sys,
        messages:finalMsgs.length?finalMsgs:[{role:'user',content:text}]
      })
    });
    const rData=await res.json();
    thinkDiv.remove();
    let reply='';
    if(!rData.ok) reply='âš ï¸ '+(rData.msg||'Tidak bisa menjawab sekarang.');
    else reply=rData.reply||'Maaf, tidak ada jawaban.';
    ...
    const formatted=escHtml(reply)
      ...
      .replace(/\*(.*?)\*/gs,'<em>$1</em>')
      .replace(/```[\s\S]*?```/g,m=>`<code style="background:rgba(255,255,255,.1);padding:2px 6px;border-radius:4px;font-size:12px;">${escHtml(m.replace(/```/g,'').trim())}</code>`)
      .replace(/\n\n/g,'<br><br>')
      .replace(/\n/g,'<br>');
    aDiv.innerHTML=`<div class="ai-msg-av">ðŸ¤–</div><div><div class="ai-bubble">${formatted}</div></div>`;

    // Show "copy to chat" button if relevant
    ...
    if(currentFriend&&isReplyHelp){
      const btn=document.createElement('button');
      btn.className='copy-to-chat-btn';
      btn.innerHTML='ðŸ“‹ Salin ke chat';
      ...
      btn.onclick=()=>{
        ...
        closeAI();
        document.getElementById('msg-input').focus();
        toast('Pesan disalin ke chat âœ…');
      };
    ...
    errDiv.innerHTML=`<div class="ai-msg-av">ðŸ¤–</div><div class="ai-bubble" style="background:rgba(220,38,38,.1);border-color:rgba(220,38,38,.3);color:#fca5a5;">Terjadi kesalahan koneksi. Coba lagi ðŸ˜”<br><small style="opacity:.7">${escHtml(String(e))}</small></div>`;
    container.appendChild(errDiv); container.scrollTop=container.scrollHeight;
  }
}

// ============================================================
// NAVIGATION STACK (Tombol Kembali per tahap)
// ============================================================
// Stack: 'home' | {type:'chat',friend} | {type:'status_view'} | {type:'overlay',id}
let navStack = ['home'];

function navPush(state){
  // Don't push duplicate top state
  const top=navStack[navStack.length-1];
  if(top&&typeof top==='object'&&state&&typeof state==='object'&&top.type===state.type&&top.id===state.id)return;
  navStack.push(state);
}
...
function navBack(){
  ...
  if(openOverlays.length>0){
    ...
    // Clear status timer if status viewer was closed
    if(openOverlays[openOverlays.length-1].id==='stview-ov') clearStvTimer();
    // Sync nav stack
    const idx=navStack.findLastIndex(s=>s&&s.type==='overlay');
    if(idx>=0) navStack.splice(idx,1);
    return;
  }
  ...
  if(aiDrawer.classList.contains('open')){
    ...
    const idx=navStack.findLastIndex(s=>s&&s.type==='ai');
    if(idx>=0) navStack.splice(idx,1);
    return;
  }
  // Priority 3: go from chat back to home
  if(currentFriend){
    ...
    // Pop chat from stack
    const idx=navStack.findLastIndex(s=>s&&s.type==='chat');
    if(idx>=0) navStack.splice(idx,1);
    return;
  }
...
// Hardware back button / browser back (mobile)
