...
.stab{flex:1;padding:10px 4px;text-align:center;font-size:11px;font-weight:800;color:var(--st);
  cursor:pointer;border-bottom:2.5px solid transparent;transition:.2s;}
...
<div class="main" id="main-panel">
  ...
  <div class="no-chat" id="no-chat" style="display:flex;">
    ...
    <div style="margin-top:16px;display:flex;gap:10px;">
      <button onclick="switchTab('status');openAI()" style="padding:10px 20px;background:linear-gradient(135deg,#7c3aed,#9b5de5);color:#fff;border:none;border-radius:20px;font-size:14px;font-weight:800;cursor:pointer;display:flex;align-items:center;gap:7px;font-family:'Nunito',sans-serif;">
        ðŸ¤– Buka AI
      </button>
...
async function openMyStatus(){
  ...
  if(!myStats.length){openPanel('cstat-ov');return;}
...
async function viewFriendStatus(uid){
  ...
  if(!d.statuses||!d.statuses.length)return;
  const u=allUsers.find(x=>x.uid===uid)||{uid,username:'?',avatar:''};
...
function renderSTV(){
  ...
  if(s.type==='text'){
    ...
    body=`<div class="stv-text" style="background:${bgs[stvIdx%bgs.length]};border-radius:12px;width:100%;min-height:160px;">${escHtml(s.content)}</div>`;
  }else if(s.type==='image') body=`<img class="stv-img" src="${s.media_url}" alt="status">`;
  else if(s.type==='video') body=`<video src="${s.media_url}" controls autoplay style="max-width:100%;max-height:360px;border-radius:12px;"></video>`;
  ...
  stvTimerInt=setTimeout(()=>nextStatus(),5000);
}
...
async function sendAIMsg(text){
  ...
  try{
    ...
    sys+='Jika diminta menulis pesan untuk dikirim ke teman, buat pesan yang sesuai dan alami.';

    const messages=[{role:'user',content:sys},...aiHistory.slice(-12)];
    // Ensure no consecutive same roles after system
    const clean=[]; for(let i=1;i<messages.length;i++){if(!clean.length||messages[i].role!==clean[clean.length-1].role)clean.push(messages[i]);}

    const res=await fetch('https://api.anthropic.com/v1/messages',{
      method:'POST',
      headers:{'Content-Type':'application/json'},
      body:JSON.stringify({model:'claude-sonnet-4-20250514',max_tokens:1000,messages:clean})
    });
    const rData=await res.json(); thinkDiv.remove();
    const reply=rData.content?.[0]?.text||'Maaf, tidak bisa menjawab sekarang.';
    ...
    errDiv.innerHTML=`<div class="ai-msg-av">ðŸ¤–</div><div class="ai-bubble" style="background:rgba(220,38,38,.1);border-color:rgba(220,38,38,.3);color:#fca5a5;">Terjadi kesalahan. Periksa koneksi kamu ðŸ˜”</div>`;
