...
// Load Firebase config dari server
...
async function loginGoogle(){
  if(!fbAuth){toast('Google login tidak tersedia, hubungi admin âš ï¸',4000);return;}
  try{
    ...
    else{document.getElementById('le2').textContent=d.msg||'Google login gagal';}
  }catch(e){
    const msg=e.code==='auth/popup-closed-by-user'?'Login dibatalkan':e.message||'Google login gagal';
    document.getElementById('le2').textContent=msg;
  }
}
</script>
</body>
...
async function ctxDelete(){
  if(!ctxMsg)return;closeCtx();
  if(!confirm('Hapus pesan ini?'))return;
  const r=await fetch('/api/delete_message',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({message_id:ctxMsg.id,chat_id:"_".join_sort([ME.uid,currentFriend.uid])})});
  // Use the backend
  await fetch('/api/delete_message',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({message_id:ctxMsg.id,friend_uid:currentFriend.uid})});
  loadMessages();toast('Pesan dihapus ðŸ—‘ï¸');
}
...
async function loadStatuses(){
  try{
    ...
    if(!stats.length){list.innerHTML='<div style="text-align:center;color:var(--st);padding:20px;">Belum ada status ðŸ“­</div>';return;}
    const byUser={};
...
async function postTextStatus(){
  ...
  const d=await r.json();
  if(d.ok){toast('Status diposting! âœ…');closePanel('cstat-ov');document.getElementById('stat-txt').value='';document.getElementById('char-cnt').textContent='0/200';}
...
async function uploadStatus(input,type){
  ...
  try{
    const r=await fetch('/api/status/upload',{method:'POST',body:fd});
    const d=await r.json();done();
    if(d.ok){toast('Status diposting! âœ…');closePanel('cstat-ov');}
...
def main_app_html(u):
    ...
    html = html.replace("__USERNAME__", username.replace('"','&quot;'))
