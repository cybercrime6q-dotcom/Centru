...
.chat-header{height:60px;background:var(--pn);display:flex;align-items:center;gap:6px;
  padding:0 6px 0 4px;border-bottom:1px solid var(--bd);flex-shrink:0;}
...
.header-info{flex:1;cursor:pointer;min-width:0;padding:0 4px;}
.header-info h3{font-weight:900;font-size:15px;color:var(--tx);}
.header-info p{font-size:11px;color:var(--g);}
.header-btn{width:40px;height:40px;border-radius:50%;background:transparent;color:var(--tx);opacity:.9;
  display:flex;align-items:center;justify-content:center;transition:.2s;flex-shrink:0;}
.header-btn:hover{background:var(--hv);opacity:1;}
...
<div class="main" id="main-panel">
  ...
  <div id="chat-wrap" style="display:none;flex-direction:column;height:100%;overflow:hidden;">
    <div class="chat-header">
      ...
      <div class="header-av" id="header-av" onclick="showContactInfo()"></div>
      <div class="header-info" onclick="showContactInfo()" style="cursor:pointer;">
        <h3 id="header-name">â€”</h3>
        <p id="header-status">â€”</p>
      </div>
      <!-- Video Call button - persis seperti WhatsApp -->
      ...
      <!-- Audio Call button - persis seperti WhatsApp -->
      ...
      <!-- More options -->
      <button class="header-btn" onclick="showChatMenu(this)" title="Lainnya" id="chat-more-btn">
        <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M12 8c1.1 0 2-.9 2-2s-.9-2-2-2-2 .9-2 2 .9 2 2 2zm0 2c-1.1 0-2 .9-2 2s.9 2 2 2 2-.9 2-2-.9-2-2-2zm0 6c-1.1 0-2 .9-2 2s.9 2 2 2 2-.9 2-2-.9-2-2-2z"/></svg>
      </button>
    </div>

    <!-- Chat more menu dropdown -->
    <div id="chat-more-menu" style="display:none;position:fixed;background:var(--pn);border:1px solid var(--bd);border-radius:10px;box-shadow:0 4px 20px rgba(0,0,0,.5);z-index:300;min-width:180px;overflow:hidden;">
      <div class="ctx-item" onclick="openAI();document.getElementById('chat-more-menu').style.display='none'">ðŸ¤– Tanya AI</div>
      <div class="ctx-item" onclick="clearChatConfirm();document.getElementById('chat-more-menu').style.display='none'">ðŸ—‘ï¸ Hapus Chat</div>
      <div class="ctx-item" onclick="searchInChat();document.getElementById('chat-more-menu').style.display='none'">ðŸ” Cari Pesan</div>
    </div>
...
<div class="ai-drawer" id="ai-drawer">
  ...
  <div class="ai-messages" id="ai-messages">
    <div class="ai-msg ai">
      ...
      <div class="ai-bubble">Halo! Saya <b>WaClone AI</b> ðŸ‘‹<br><br>Saya bisa membantu:<br>â€¢ Menulis atau menyempurnakan pesan<br>â€¢ Menjawab pertanyaan apapun<br>â€¢ Menerjemahkan teks<br>â€¢ Memberi ide &amp; saran kreatif<br><br>Apa yang bisa saya bantu?</div>
    </div>
  </div>
...
<div class="overlay" id="prof-ov">
  <div class="panel">
    <div class="panel-header"><h2>ðŸ‘¤ Profil Saya</h2><button class="close-btn" onclick="closePanel('prof-ov')">âœ•</button></div>
...
<div class="overlay" id="settings-ov">
  <div class="panel" style="width:440px;">
    <div class="panel-header"><h2>âš™ï¸ Pengaturan</h2><button class="close-btn" onclick="closePanel('settings-ov')">âœ•</button></div>
    <div class="panel-body">
      ...
      <div class="settings-section">
        <h3>ðŸ—„ï¸ Data &amp; Penyimpanan</h3>
        <div class="settings-row">
          <div><div class="settings-label">Hapus Cache Lokal</div><div class="settings-desc">Bersihkan data sementara di browser</div></div>
          <button class="settings-btn" onclick="clearLocalCache()">ðŸ—‘ï¸ Hapus</button>
        </div>
        <div class="settings-row">
          <div><div class="settings-label">Versi Aplikasi</div><div class="settings-desc">WaClone v2.0 â€” Build 2025</div></div>
...
<div class="overlay" id="notif-ov">
  <div class="panel">
    <div class="panel-header"><h2>ðŸ”” Notifikasi</h2><button class="close-btn" onclick="closePanel('notif-ov');markNotifsRead()">âœ•</button></div>
    <div class="panel-body" id="notif-list"><div style="text-align:center;color:var(--st);padding:22px;">Tidak ada notifikasi ðŸŽ‰</div></div>
  </div>
</div>
...
<div class="overlay" id="cstat-ov">
  <div class="panel">
    <div class="panel-header"><h2>âœï¸ Buat Status</h2><button class="close-btn" onclick="closePanel('cstat-ov')">âœ•</button></div>
    <div class="panel-body">
...
<div class="overlay" id="cam-ov">
  <div class="panel" style="width:420px;">
    <div class="panel-header"><h2>ðŸ“· Kamera</h2><button class="close-btn" onclick="closeCamera()">âœ•</button></div>
    <div class="panel-body">
...
<div class="overlay" id="fw-ov">
  <div class="panel">
    <div class="panel-header"><h2>â†ªï¸ Teruskan Pesan</h2><button class="close-btn" onclick="closePanel('fw-ov')">âœ•</button></div>
    <div class="panel-body">
...
<!-- ===== CALL UI â€” FIXED video call layout ===== -->
<div class="call-ui" id="call-ui">
  <!-- Video grid â€” hidden by default, shown only for video calls -->
  ...
  <!-- Audio call info â€” shown for audio calls -->
  ...
  <!-- Controls bar â€” camera button uses JS to show/hide -->
...
const EMOJIS = ['ðŸ˜€','ðŸ˜‚','ðŸ˜','ðŸ¥°','ðŸ˜Ž','ðŸ¤”','ðŸ˜­','ðŸ˜¡','ðŸ‘','ðŸ‘Ž','â¤ï¸','ðŸ”¥','âœ…','â­','ðŸŽ‰','ðŸ™',
...
function showContactInfo(){
  ...
  // simple info toast
  toast((u?.username||currentFriend.name)+' â€” '+(bio||'No bio'),3000);
}
...
// FIX: openMyStatus â€” setelah post status, langsung refresh dan tampilkan viewer
async function openMyStatus(){
  try{
    const r=await fetch('/api/status/my'); const d=await r.json();
    const myStats=d.statuses||[];
    if(!myStats.length){
      // Tidak ada status â€” buka buat status, tutup viewer dulu
      closePanel('stview-ov');
      openPanel('cstat-ov');
      return;
    }
    // Ada status â€” buka viewer
    closePanel('cstat-ov');
...
async function answerCall(type){
  ...
  document.getElementById('call-status-txt').style.color='#00a884';
  startCallTimer(); pollICE();
  if(type==='video'&&appSettings.auto_spk) toast('Speaker aktif ðŸ”Š',1500);
}
...
function showCallUI(friend,type,dir){
  ...
  document.getElementById('cbtn-spk').textContent='ðŸ”Š'; document.getElementById('cbtn-spk').className='call-cbtn cbtn-toggle cbtn-on';
}
...
// ============================================================
// AI ASSISTANT â€” FIXED alternating roles + better error handling
// ============================================================
function openAI(){
...
async function sendAIMsg(text){
  ...
  try{
    ...
    // FIX: Build valid alternating messages â€” harus dimulai user, bergantian user/assistant
    ...
    let reply='';
    if(!rData.ok){
      reply='âš ï¸ '+(rData.msg||'Tidak bisa menjawab sekarang. Coba lagi!');
    ...
    errDiv.innerHTML=`<div class="ai-msg-av">ðŸ¤–</div><div class="ai-bubble" style="background:rgba(220,38,38,.1);border-color:rgba(220,38,38,.3);color:#fca5a5;">Terjadi kesalahan koneksi. Coba lagi ðŸ˜”</div>`;
...
def api_ai_chat():
    ...
    # Validate and fix messages: must start with user, alternate user/assistant
    ...
    try:
        ...
        try:
            err_json = __import__('json').loads(err_body)
            err_msg = err_json.get("error",{}).get("message", err_body[:200])
        except:
            err_msg = err_body[:200]
        return jsonify({"ok": False, "msg": "AI error: " + err_msg})
    except Exception as e:
