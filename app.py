...
async function handleUpload(input){
  ...
  try{
    ...
    else toast('Upload gagal: '+(d.msg||'Error tidak diketahui'),3500,true);
...
async function sendCamPhoto(){
  ...
  if(camMode==='status'){
    ...
    if(!d.ok){toast('Upload gagal: '+(d.msg||''),3000,true);return;}
...
async function viewFriendStatus(uid){
  try{
    ...
    // Cari di allUsers, fallback ke data dari status itu sendiri
    const uFound=allUsers.find(x=>x.uid===uid);
    const firstStat=d.statuses[0];
    const u=uFound||{uid, username: firstStat.username||firstStat.display_name||uid.slice(0,8), avatar: firstStat.avatar||''};
    stvData={user:u,statuses:d.statuses}; stvIdx=0; renderSTV(); openPanel('stview-ov');
  }catch(e){toast('Gagal memuat status: '+e.message,2500,true);}
}
...
async function uploadStatus(input,type){
  ...
  if(file.size>20*1024*1024){toast('Ukuran file max 20MB',2500,true);return;}
  const done=showUploadBar(),fd=new FormData();
  ...
  try{
    ...
    if(d.ok){
      ...
      await loadSidebarStatuses();
      setTimeout(openMyStatus,600);
    }else toast('Upload status gagal: '+(d.msg||''),3000,true);
  }catch(e){done();toast('Upload error: '+e.message,3000,true);}
  input.value='';
}
...
async function uploadAvatar(input){
  ...
  try{
    ...
    if(!d.ok){toast('Upload foto gagal: '+(d.msg||''),3000,true);return;}
    if(d.ok){
...
function checkMobile(){
  ...
  if(bb) bb.style.cssText=isMobile
    ? 'display:flex!important;flex-shrink:0;width:36px;height:36px;border:none;background:transparent;color:#e9edef;border-radius:50%;align-items:center;justify-content:center;cursor:pointer;'
...
def api_status_list():
    ...
    try:
        ...
        # Enrich dengan username/avatar dari users collection
...
def api_status_user(uid):
    ...
    try:
        ...
        print("status_user error:", e, file=sys.stderr)
        return jsonify({"statuses":[]})
